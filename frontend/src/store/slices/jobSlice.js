...
export const fetchJobs =
  (city, niche, searchKeyword = "", salary) =>
  async (dispatch) => {
    try {
      ...
      // EXACT MATCH: Append salary filter as a string
      if (salary && salary !== "All") {
        queryParams.push(`salary=${salary}`);
      }

      if (city && city === "All") {
        // Do not reset queryParams, just skip adding city
      } else if (city) {
        queryParams.push(`city=${city}`);
      }
      ...
      if (niche && niche === "All") {
        // Do not reset queryParams, just skip adding niche
      } else if (niche) {
        queryParams.push(`niche=${niche}`);
      }
